...
pop_lg = function (Frs.spp,nr.spp, sr.spp,alpha.spp,kd.spp,kc.spp){
	...
	cr_tot = matrix(0,np,nspp)
		for (sa in 1:nspp) { 
			for (sb in 1:nspp) {
					cr[,(sa*(sa-1)+sb)] = Re(fft((alphas[sa,sb]*fp[,sb]*fkc[,sb]),
						inverse=T)/(np+1))
...
#Get the invader's equilibrium density using simulations. This version does
#not require any analytical calculations and works for an arbitrary number 
#of species. 
#1D array of sites. 
#It takes as variables: 
...
get.inv.eq = function(Frs.spp, nr.res,s.inv,sr.spp,alpha.spp,kd.spp,kc.spp,burns=500, burns.max = 10,tol=1,fast=FALSE,fast.tol=1e-5){

	np = dim(Frs.spp)[1] #Space
	nspp = dim(Frs.spp)[2] #Number of species
	s.index= 1:nspp
	s.index = s.index[-s.inv]
	...
	while (b <= burns.max){ 
		...
		for (n in 1:(burns-1)) {
			alpha_inv = alpha.spp
			#alpha_inv[, s.inv] = matrix(0, nspp,1 )
			#nr.burns[(n+1),,s.inv] = pop_lg(Frs.spp ,nr.burns[n,,], sr.spp,alpha_inv,kd.spp,kc.spp)[,s.inv]
			nr.burns[(n+1),,s.inv] = pop_lg_inv(Frs.spp , nr.burns[n,,],s.inv, sr.spp,alpha_inv,kd.spp,kc.spp)
...
get.isd.pair = function(Fr.spp,nr.spp, sr.spp,alpha.ir,kd.spp,kc.spp ){
	...
	fkc.spp = fft( c((1:ceiling(np/2)*0), kc.spp, (1:ceiling(np/2)*0) )) 
	#Transform resident stationary distribution
	fnr = fft(c((1:ceiling(np/2)*0), nr.spp/(mean(nr.spp))-1, (1:ceiling(np/2)*0) ))
	...
	for( b in seq(10,1,-1)) { 
			...
			Bu = (lam1*fkd.spp+sr.spp)^(b-1)
			Be = lam1*fkd.spp
			Bur = -lam1*fkd.spp*fkc.spp*(alpha.ir*n.ave)/D.spp
			ir_tmp.f = ir_tmp.f + Bu*(Be*eft+Bur*fnr)
		}
...
get.fd.cov.p = function (Fr.spp, nr.spp, sr.spp, alpha.ir, a_rr, kc.spp){ 
	...
	uijmuj = uijmuj+abs(min(uijmuj))

	D.spp = 1+1/(alpha.ir*rmcc2)
	...
	#Then multiply by average igr
	#agr1=(gr1.spp-sr.spp)
	agr1= rmFr1/(D.spp*alpha.ir*rmcc2*2*3.14)
	return(Re(fdc2[1])*agr1)
...
get.fd.cov.p2 = function (Fr.spp,nr.spp, sr.spp,alpha.ir,kd.spp,kc.spp ){
	...
	#Fitness-density covariance, standardized 
	fdcv2 = rmFr1/(D.spp*alpha.ir*rmcc2) * cov(e1,isd.tmp)
