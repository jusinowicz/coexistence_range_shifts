...
#For naming files
#=============================================================================
f.name1=c("igr_10spp_s1000_t10000f")
...
ngens=3 #Number of generations for environmental change
...
Fr=matrix(abs(rnorm(nspp,2,2)),nspp,1)  
...
# lower group move into upper group. 
	...
	# mean.lower = rnorm(nspp/2, -150, 100)
	# mean.upper = rnorm(nspp/2, 150, 100)
	# peaks=c(mean.lower, mean.upper  )
	# #Randomly choose which species move:  
	# mean.trans = (runif(nspp/2)>0.2)
	# peaks.end=c((mean.lower+mean.upper*mean.trans), mean.upper); f.name2=c("_trans")

	mean.lower = rnorm(nspp/2, -200, 100)
	mean.upper = rnorm(nspp/2, 100, 100)
	peaks=c(mean.lower, mean.upper  )
	#Randomly choose which species move:  
	mean.trans = (runif(nspp)>0.3)
	peaks.end=peaks+mean.upper*mean.trans; f.name2=c("_trans")
...
frs_all =c(list(peaks), list(peaks.end),list(Ds),list(Ds.end))
save(file=file.name, "frs_all", "l1", "D", "var_mu_Us","cov_e_mu_Us",
