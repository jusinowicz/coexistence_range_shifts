...
ns=2000 #Lattice width/height -- This should be even
ngens=50 #Number of generations for environmental change
...
Fr=matrix(c(2.5,1,1.5)nspp,1) #Peak height
...
# lower group move into upper group. 
	...
	# 1. peaks (spatial mean)
	peaks=c(-250,-150 ,-50)*2.5
	peaks.end=c(200, -150, 200)*2.5


	# 2. Ds
	Ds = c(1000^2/100, 1000^2/150,1000^2/200)
	Ds.end = c( 1000^2/50,1000^2/25,1000^2/200)
...
fig.name = paste("sim_rangeshifts_3sppC.pdf",sep="")
...
abline(h=0)
for(j in 2:nspp) {
	lines( Frs[1,,j],col = color.use[j],lwd=3,lty=2)
}
...
abline(h=0)
for(j in 1:nspp) {
	lines( Frs[ngenst,,j],col = color.use[j],lwd=3)
}
...
"ldgIMP.sim.p","covIMP.sim.p")


#List of scenario file names 
file.name.list= list("igr_3spp_s1000_t10000D_trans_novar_norm.var", "igr_3spp_s1000_t10000D_notrans_var_norm.var",
	"igr_3spp_s1000_t10000D_trans_var_norm.var", "igr_3spp_s1000_t10000E_trans_novar_fast.var" )
...
#fig.name = paste("all4_linear_widthOverlap_2Dgr4.pdf",sep="")
#pdf(file=fig.name, height=7.5, width=7.5, onefile=TRUE, family='Helvetica', pointsize=16)
...
lines((seq( (-ns/2),(ns/2)))[Frs[tol2[j],,1]>1e-2]-j1[1]*15, (Frs[tol2[j],,1][Frs[tol2[j],,1]>1e-2]),col = color.use[[1]],lwd=2) 
...
####################################
###############################################################################
fig.name = paste("fails_to_track_2Dgr1B.pdf",sep="")
pdf(file=fig.name, height=7.5, width=7.5, family='Helvetica', pointsize=16)
###############################################################################
par(mfrow=c(2,4),mai= c( 1, 0, 0.2, 0), omi=c(2,0.75,2,0.75))
###############################################################################
...
j=4
plot( 1:np, (Frs[tol2[j],,1]), xlim=c(1,np), ylim=c(0, max(Frs[tol2[j],,1])+0.0005),
	type="l", ylab="", xlab="", cex.lab=1.7,cex.axis=1.2, col=color.use[[1]],
	xaxs="i",xaxt='n',yaxs="i",yaxt="n",bty="n",lwd = 2 )
	
axis(side=2, labels=T, at=c(max(Frs[tol2[j],,1])+0.0005) )
abline(h=0)
abline(v=1)
#Shade the green distribution
bottom=matrix(0,length(1:np),1)
polygon(c(1:np, rev(1:np)), c(Frs[tol2[j],,1], rev(bottom) ),col = adjustcolor("darkgreen", alpha.f=0.4), border = NA)

# for(j in 2:nspp) {
# 	lines(1:np, (Frs[tol2[j],,j]),col = color.use[[j]],lwd=2)
# }
...
#List of scenario file names 
file.name.list= list("igr_10spp_s1000_t10000f_trans_var_norm.var")

load(file.name.list[[1]])
...
fig.name = paste("allN_ranges_2Dgr1G.pdf",sep="")
pdf(file=fig.name, height=7.5, width=7.5, family='Helvetica', pointsize=16)
###############################################################################
...
plot( (seq( (-ns/2),(ns/2)))[Frs[1,,1]>1e-2], (Frs[1,,1][Frs[1,,1]>1e-2]), xlim=c((-ns/2),(ns/2)), 
	...
	xaxs="i",xaxt='n',yaxs="i",yaxt='n',bty="n",lwd = 2,lty=2 )
abline(h=0)
for(j in 1:nspp) {
	lines((seq( (-ns/2),(ns/2)))[Frs[1,,j]>1e-2], (Frs[1,,j][Frs[1,,j]>1e-2]),col = color.use[[j]],lwd=2,lty=2)
	
}

plot( (seq( (-ns/2),(ns/2)))[Frs[tol2,,1]>1e-2], (Frs[tol2,,1][Frs[tol2,,1]>1e-2]), xlim=c((-ns/2),(ns/2)), 
	ylim=c(0,6.5), type="l", ylab="", xlab="", cex.lab=1.7,cex.axis=1.2, col=color.use[[1]],
	xaxs="i",xaxt='n',yaxs="i",yaxt='n',bty="n",lwd = 2)
abline(h=0)
for(j in 1:nspp) {

lines((seq( (-ns/2),(ns/2)))[Frs[tol2,,j]>1e-2], (Frs[tol2,,j][Frs[tol2,,j]>1e-2]),col = color.use[[j]],lwd=2) 
}
