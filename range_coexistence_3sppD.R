...
#For naming files
#=============================================================================
f.name1=c("igr_3spp_s1000_t10000E")
...
ngens=5 #Number of generations for environmental change
...
#same starting place in space for lower 2. 
	...
	peaks.end=c(floor(np), -50, 150) #Make this large to ensure fast shifting
	#deltaE=100 # Rate of shift
...
for ( s in 1:nspp) { 
	...
	for( t in 1: ngenst){ 
		...
		for (sp in 1:(nspp-1)){ 
			...
			nr.p[t,,s.index[sp]] = get.res.eq(Frs[t,,],s.index[sp],sr,alphas, kd,kc,fast=TRUE )
		}
		...
		for (sp in 1:(nspp-1)){ 
			...
			ni.p[t,,s.index[sp]] =get.inv.ldeq(Frs[t,,sort(c(s,s.index[sp]))], nr.p[t,,sort(c(s,s.index[sp]))], s.inv, sr,alphas, kd,kc,fast=TRUE )
		}
